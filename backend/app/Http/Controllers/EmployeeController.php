...
class EmployeeController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $role = in_array($request->role, ['employee', 'hr_manager']) ? $request->role : 'employee';
        ...
        $newUser = [
            ...
            'role' => $role, 'tenant_id' => $tenantId, 'department' => $request->department ?? '',
            'position' => $request->position ?? '', 'salary' => (float)($request->salary ?? 0),
    ...
    public function update(Request $request, string $employeeId)
    {
        $user = $request->auth_user;
        if (!in_array($user['role'], ['super_admin', 'hr_manager'])) return response()->json(['detail' => 'Not authorized'], 403);
        $updates = array_filter($request->only(['name', 'department', 'position', 'salary', 'mobile', 'status']), fn($v) => $v !== null);
